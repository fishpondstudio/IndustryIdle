...
import {
	EntityDefaultType,
	getEntityDefault,
} from "../CoreGame/Logic/EntityDefault";
...
export type IBoostCache = Record<
	string,
	Partial<Record<keyof Buildings, [number, number]>>
>;
...
export class PersistedData {
	...
	achievements: Partial<
		Record<keyof typeof ACH, { achieved: boolean; claimed: boolean }>
	> = {};
	cf = false;
...
export const FontSizeScalingOptions = [
	"0.9",
	"1",
	"1.1",
	"1.2",
...
function convertSKUsToDLCs(
	purchases: string[],
): Partial<Record<DownloadableContent, true>> {
...
export const PlatformSKUs: Partial<
	Record<Platform, Record<string, DownloadableContent>>
> = {
	iOS: {
		ep1: DLC[0],
		ep2: DLC[1],
...
export function signTrade<T extends { sig: string; token: string }>(
	data: T,
): T {
	delete data.sig;
...
export function encryptData<T>(data: T) {
	return CryptoJS.AES.encrypt(JSON.stringify(data), SAVE_FILE_KEY).toString(
...
export async function authenticatePlayer(): Promise<Response> {
	return fetch(
		`${API_HOST}/user/authentication?${await getAuthQueryString()}`,
		{
			method: "post",
...
export const API_HOST = serverOverride
	? `http://${serverOverride}`
	: "https://api.fishpondstudio.com";
...
export function checkVerification() {
	return fetch(
		`${API_HOST}/user/verification?tradeToken=${D.persisted.tradeToken}`,
		{
			method: "post",
			headers: { "X-User-Id": D.persisted.userId },
		},
...
export function forceRestoreTradeToken() {
	return fetch(
		`${API_HOST}/trade/force-restore?token=${D.persisted.tradeToken}`,
		{
			method: "post",
			headers: { "X-User-Id": D.persisted.userId },
		},
